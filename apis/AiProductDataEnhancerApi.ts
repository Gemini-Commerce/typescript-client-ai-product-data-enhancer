...
export class AiProductDataEnhancerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param body 
     */
    public async aiProductDataEnhancerFillProductDataCheck(body: AiproductdataenhancerFillProductDataCheckRequest, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'body' is not null or undefined
        if (body === null || body === undefined) {
            throw new RequiredError("AiProductDataEnhancerApi", "aiProductDataEnhancerFillProductDataCheck", "body");
        }


        // Path Params
        const localVarPath = '/aiproductdataenhancer.AiProductDataEnhancer/FillProductDataCheck';
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(body, "AiproductdataenhancerFillProductDataCheckRequest", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class AiProductDataEnhancerApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to aiProductDataEnhancerFillProductDataCheck
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async aiProductDataEnhancerFillProductDataCheckWithHttpInfo(response: ResponseContext): Promise<HttpInfo<AiproductdataenhancerFillProductDataCheckResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AiproductdataenhancerFillProductDataCheckResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "AiproductdataenhancerFillProductDataCheckResponse", ""
            ) as AiproductdataenhancerFillProductDataCheckResponse;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("0", response.httpStatusCode)) {
            const body: RpcStatus = ObjectSerializer.deserialize(
